 ...
 */
(function(jQuery){jQuery.fn.dataTableSettings=[];var _aoSettings=jQuery.fn.dataTableSettings;jQuery.fn.dataTableExt={};
var _oExt=jQuery.fn.dataTableExt;_oExt.sVersion="1.6.2";_oExt.iApiIndex=0;_oExt.oApi={};
...
jQuery(nPrevious).click(function(){if(oSettings.oApi._fnPageChange(oSettings,"previous")){fnCallbackDraw(oSettings)
}});jQuery(nNext).click(function(){if(oSettings.oApi._fnPageChange(oSettings,"next")){fnCallbackDraw(oSettings)
}});jQuery(nPrevious).bind("selectstart",function(){return false});jQuery(nNext).bind("selectstart",function(){return false
...
nPaging.appendChild(nLast);jQuery(nFirst).click(function(){if(oSettings.oApi._fnPageChange(oSettings,"first")){fnCallbackDraw(oSettings)
}});jQuery(nPrevious).click(function(){if(oSettings.oApi._fnPageChange(oSettings,"previous")){fnCallbackDraw(oSettings)
}});jQuery(nNext).click(function(){if(oSettings.oApi._fnPageChange(oSettings,"next")){fnCallbackDraw(oSettings)
}});jQuery(nLast).click(function(){if(oSettings.oApi._fnPageChange(oSettings,"last")){fnCallbackDraw(oSettings)
}});jQuery("span",nPaging).bind("mousedown",function(){return false}).bind("selectstart",function(){return false
});if(oSettings.sTableId!==""&&typeof oSettings.aanFeatures.p=="undefined"){nPaging.setAttribute("id",oSettings.sTableId+"_paginate");
...
jQuery.fn.dataTable=function(oInit){function classSettings(){this.fnRecordsTotal=function(){if(this.oFeatures.bServerSide){return this._iRecordsTotal
...
this.sAjaxSource=null;this.bAjaxDataGet=true;this.fnServerData=jQuery.getJSON;this.iServerDraw=0;
...
nNewCell.innerHTML=sHtml;var nTrs=jQuery("tbody tr",oSettings.nTable);if(jQuery.inArray(nTr,nTrs)!=-1){jQuery(nNewRow).insertAfter(nTr)
...
}var nTrHead=jQuery("thead:eq(0)>tr",oSettings.nTable)[0];var nTrFoot=jQuery("tfoot:eq(0)>tr",oSettings.nTable)[0];
...
}if(!oCol.bSortable||(jQuery.inArray("asc",oCol.asSorting)==-1&&jQuery.inArray("desc",oCol.asSorting)==-1)){oCol.sSortingClass=oSettings.oClasses.sSortableNone;
oCol.sSortingClassJUI=""}else{if(jQuery.inArray("asc",oCol.asSorting)!=-1&&jQuery.inArray("desc",oCol.asSorting)==-1){oCol.sSortingClass=oSettings.oClasses.sSortableAsc;
oCol.sSortingClassJUI=oSettings.oClasses.sSortJUIAscAllowed}else{if(jQuery.inArray("asc",oCol.asSorting)==-1&&jQuery.inArray("desc",oCol.asSorting)!=-1){oCol.sSortingClass=oSettings.oClasses.sSortableDesc;
...
}nTr.appendChild(nTh)}}jQuery("thead:eq(0)",oSettings.nTable).html("")[0].appendChild(nTr)
...
}else{jQuery(oSettings.aoColumns[i].nTh).addClass(oSettings.oClasses.sSortableNone)}}jQuery("thead:eq(0) th",oSettings.nTable).mousedown(function(e){if(e.shiftKey){this.onselectstart=function(){return false
...
if(aoData._sRowStripe!=sStrip){jQuery(nRow).removeClass(aoData._sRowStripe).addClass(sStrip);
...
anRows[iRowCount].appendChild(nTd)}if(typeof oSettings.fnHeaderCallback=="function"){oSettings.fnHeaderCallback(jQuery("thead:eq(0)>tr",oSettings.nTable)[0],_fnGetDataMaster(oSettings),oSettings._iDisplayStart,oSettings.fnDisplayEnd(),oSettings.aiDisplay)
}if(typeof oSettings.fnFooterCallback=="function"){oSettings.fnFooterCallback(jQuery("tfoot:eq(0)>tr",oSettings.nTable)[0],_fnGetDataMaster(oSettings),oSettings._iDisplayStart,oSettings.fnDisplayEnd(),oSettings.aiDisplay)
...
nFilter.innerHTML=oSettings.oLanguage.sSearch+sSpace+'<input type="text" />';var jqFilter=jQuery("input",nFilter);
jqFilter.val(oSettings.oPreviousSearch.sSearch.replace('"',"&quot;"));jqFilter.keyup(function(e){var n=oSettings.aanFeatures.f;
for(var i=0,iLen=n.length;i<iLen;i++){if(n[i]!=this.parentNode){jQuery("input",n[i]).val(this.value)
...
}var asSearch=bEscapeRegex?_fnEscapeRegex(sInput).split(" "):sInput.split(" ");var sRegExpString="^(?=.*?"+asSearch.join(")(?=.*?")+").*jQuery";
...
_fnCalculateEnd(oSettings);_fnDraw(oSettings)}}function _fnSortAttachListener(oSettings,nNode,iDataIndex,fnCallback){jQuery(nNode).click(function(e){if(oSettings.aoColumns[iDataIndex].bSortable===false){return
...
for(i=0;i<iColumns;i++){if(oSettings.aoColumns[i].bSortable){jQuery(oSettings.aoColumns[i].nTh).removeClass(oClasses.sSortAsc+" "+oClasses.sSortDesc+" "+oSettings.aoColumns[i].sSortingClass)
...
iFound=j;break}}jQuery(oSettings.aoColumns[i].nTh).addClass(sClass);if(oSettings.bJUI){var jqSpan=jQuery("span",oSettings.aoColumns[i].nTh);
...
}else{sSpanClass=oClasses.sSortJUIDesc}}jqSpan.addClass(sSpanClass)}}else{jQuery(oSettings.aoColumns[i].nTh).addClass(oSettings.aoColumns[i].sSortingClass)
...
jQuery("select",nLength).change(function(e){var iVal=jQuery(this).val();var n=oSettings.aanFeatures.l;
for(var i=0,iLen=n.length;i<iLen;i++){if(n[i]!=this.parentNode){jQuery("select",n[i]).val(iVal)
...
var i;var oHeaders=jQuery("thead:eq(0)>th",oSettings.nTable);for(i=0;i<iColums;i++){if(oSettings.aoColumns[i].bVisible){iVisibleColumns++;
...
var oNodes=jQuery("tr:eq(1)>td",nCalcTmp);var iIndex;for(i=0;i<oNodes.length;i++){iIndex=oNodes[i].getAttribute("tag_index");
var iContentWidth=jQuery("td",nCalcTmp).eq(i).width();var iSetWidth=oSettings.aoColumns[i].sWidth?oSettings.aoColumns[i].sWidth.slice(0,-2):0;
...
}function _fnEscapeRegex(sVal){var acEscape=["/",".","*","+","?","|","(",")","[","]","{","}","\\","jQuery","^"];
var reReplace=new RegExp("(\\"+acEscape.join("|\\")+")","g");return sVal.replace(reReplace,"\\jQuery1")
...
jQuery.getJSON(oSettings.oLanguage.sUrl,null,function(json){_fnLanguageProcess(oSettings,json,true)
