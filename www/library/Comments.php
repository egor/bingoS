...
class Comments
{
    ...
    protected function _init()
    {
        if (!empty($this->tpl)) {
            ...
            $printOText = $this->db->fetchAll("SELECT `id` FROM `comments` WHERE `goods_artikul`='".$this->artikul."' AND `visible`='1'");
            //var_dump($printOText);
            ...
            $this->tpl->assign(array(
                ...
                'COMMENTS_GOODS_HEADER' => (empty($printOText)?'':$this->header),
                'COMMENTS_FORM_ACTION' => $action,
                'CAPTCHA_ID' => $this->getCapchaId(),
                'COMMENTS_GOODS_MAIN_HEADER'=>(empty($printOText)?'':'Отзывы:'),
            ));
